...
新上传图片生成最长边 480px、最大 64 KiB 的预览，随项目写入 `meta.thumbnailPath`；主图已满足预览限制时直接复用，不重复上传。目录只在卡片接近视口时加载预览，最多两个并发请求，缓存最多 100 张；退出时释放对象 URL。旧项目没有预览时显示图片占位图标，不批量下载原图。预览生成或上传失败仍允许保存已优化的主图。
