(function (root) {
  ...
  const itemsFor = kind => kind === "arrival" ? orderItems : kind === "morning" ? items.slice(0, -1) : items;
  ...
  const empty = (kind = "count") => ({ stock: (kind === "arrival" ? orderItems : items).map(() => ""), checked: (kind === "arrival" ? orderItems : items).map(() => false), orders: orderItems.map(() => ""), units: (kind === "morning" ? items : orderItems).map(() => kind === "morning" ? "kg" : ""), updatedAt: "" });
  ...
  function setValue(draft, mode, index, value) {
    if (!Number.isInteger(index) || index < 0 || index >= (mode === "orders" ? orderItems.length : draft.stock.length) || !["stock", "orders"].includes(mode)) return false;
  ...
  function missing(draft, kind = "count") {
    return itemsFor(kind).map((_, i) => i).filter(i => !valid(draft.stock[i]));
  }
  function report(draft, kind = "count") {
    const confirmed = i => valid(draft.stock[i]);
    if (kind === "arrival" || kind === "morning") {
      const lines = itemsFor(kind).flatMap((name, i) => {
        if (!confirmed(i) || (kind === "morning" && Number(draft.stock[i]) === 0)) return [];
        const value = `${normalize(draft.stock[i])}${kind === "arrival" ? "kg" : draft.units[i]}`;
        return [`${name}：${value}`];
      });
  ...
  const api = { workflows, items, itemsFor, orderItems, units, valid, normalize, empty, initial, restore, setValue, confirmStock, missing, report };
