...
function fixture(wasEdited=()=>false) {
  const state={currentDate:'2026-09-09',predictionDate:'',products:[{id:1,last_purchased_date:'2026-04-19'}],predictions:{},cloudPurchaseDates:{}};
...
test('history reads all pages and replaces a stale 143-day badge with the newest date',async()=>{
  ...
  await f.api.fetch();assert.equal(f.state.cloudPurchaseDates[1],'2026-09-08');
...
test('today contributes to the history badge, but not to today quantity prediction',async()=>{
  ...
  await f.api.fetch();assert.equal(f.state.cloudPurchaseDates[1],'2026-09-09');
  ...
  assert.equal(f.state.cloudPurchaseDates[1],'2026-09-06');
  f.data.jlhcdh_cart=[];f.api.invalidate();await f.api.fetch();assert.equal(f.state.cloudPurchaseDates[1],undefined);
});
test('live cart days and newest legacy snapshots do not resurrect deleted entries',async()=>{
  ...
  ];await f.api.fetch();assert.equal(f.state.cloudPurchaseDates[1],'2026-09-06');
});
test('failed reads preserve last good data and are eligible for retry',async()=>{
  ...
  assert.equal(f.state.cloudPurchaseDates[1],undefined);
  f.fail(false);f.data.jlhcdh_cart=[{product_id:1,quantity:1,order_date:'2026-09-08'}];
  await f.api.fetch();assert.equal(f.state.cloudPurchaseDates[1],'2026-09-08');
  f.fail(true);f.api.invalidate();await f.api.fetch();
  assert.equal(f.state.cloudPurchaseDates[1],'2026-09-08');
});
test('purchase badge ignores cached product dates and local edit markers',async()=>{
  const f=fixture(()=>true);
  assert.equal(f.state.cloudPurchaseDates[1],undefined);
  assert.match(source,/getHistoryLabel\(state\.cloudPurchaseDates\[p\.id\]\)/);
  f.data.jlhcdh_orders=[{created_at:'2026-09-08T10:00:00Z',order_data:[{id:1,qty:2}]}];
  await f.api.fetch();
  assert.equal(f.state.cloudPurchaseDates[1],'2026-09-08');
  assert.equal(label(f.state.cloudPurchaseDates[1]),'昨天买过');
});
