...
for(let i=0;i<30 && await p.locator('#zoomIn').isEnabled();i++)await p.locator('#zoomIn').click();
if(await p.locator('#zoomValue').innerText()!=='600%')throw Error('Desktop maximum zoom is not 600%');
await assertSharpText();
...
const eye=p.locator('#toggleAnnotations');
const eyeBounds=await eye.boundingBox();
if(eyeBounds.width<44 || eyeBounds.height<44)throw Error('Visibility touch target is too small');
await eye.focus();await eye.press('Space');
if(await eye.getAttribute('aria-pressed')!=='false')throw Error('Keyboard visibility toggle failed');
if(await p.locator('#labelLayer text').count())throw Error('Hidden labels still rendered');
const hiddenImage=await p.locator('#measureCanvas').evaluate(canvas=>canvas.toDataURL());
if(hiddenImage===screenImage)throw Error('Dimension lines were not hidden');
const overlayVisible=await p.locator('#selectionCanvas').evaluate(canvas=>canvas.getContext('2d').getImageData(0,0,canvas.width,canvas.height).data.some((v,i)=>i%4===3&&v));
if(overlayVisible)throw Error('Selection remains visible with annotations hidden');
await p.locator('#zoomIn').click();
if(await p.locator('#labelLayer text').count())throw Error('Zoom revealed hidden labels');
await p.locator('#exportImage').click();
if(await eye.getAttribute('aria-pressed')!=='false')throw Error('Export changed visibility preference');
await eye.click();
if(!(await p.locator('#labelLayer text').count()))throw Error('Labels did not return');
await p.locator('#zoomIn').click();
await p.locator('#exportImage').click();
...
return {passed:true,checks:['eye visibility toggle, keyboard, 44px target and export while hidden','dense labels','sharp 13px vector text at fit and maximum zoom','displaced label selection','mobile layout','zoom-independent export','dark and increased contrast render']};
